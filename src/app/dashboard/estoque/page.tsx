...
const dadosIniciais: ItemEstoque[] = [
	{
		id: "1",
		codigo: "PÇ-001",
		nome: "Fusível de Ação Rápida 5A",
		categoria: "Elétrica",
		quantidade: 4,
		minimo: 10,
		unidade: "un",
	},
	{
		id: "2",
		codigo: "PÇ-002",
		nome: "Tubo de Silicone para Bomba de Infusão",
		categoria: "Mangueiras/Conexões",
		quantidade: 15,
		minimo: 5,
		unidade: "m",
	},
	{
		id: "3",
		codigo: "PÇ-003",
		nome: "Kit O-Ring para Autoclave",
		categoria: "Vedação",
		quantidade: 2,
		minimo: 3,
		unidade: "kit",
...
export default function EstoquePage() {
	const [itens, setItens] = useState<ItemEstoque[]>(dadosIniciais);
			...
			setItens(
				itens.map((i) =>
					i.id === itemEmEdicao.id
						? { ...i, codigo, nome, categoria, quantidade, minimo, unidade }
						: i,
				),
			);
		} else {
			...
			<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
				...
				<div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
					...
					<h3 className="text-2xl font-bold text-red-600">
						{itens.filter((i) => i.quantidade <= i.minimo).length}
					</h3>
				</div>
				<div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
					<span className="text-sm text-gray-500">Unidades Cadastradas</span>
					<h3 className="text-2xl font-bold text-blue-600">
						{itens.reduce((acc, curr) => acc + curr.quantidade, 0)}
					</h3>
				</div>
			</div>
