...
import { listarEquipamentos, salvarEquipamento, deletarEquipamento } from '@/lib/firebase'
...
export default function EquipamentosDashboardPage() {
  ...
  function verificarStatusCalibracao(eq: any) {
    ...
    if (diffDays < 0) {
      return { texto: 'Calibração Vencida', cor: 'bg-red-100 text-red-700 border-red-200' }
    } else if (diffDays <= 30) {
      return { texto: `Vence em ${diffDays} dias`, cor: 'bg-amber-100 text-amber-800 border-amber-200' }
    } else {
      return { texto: 'Calibração em Dia', cor: 'bg-emerald-100 text-emerald-700 border-emerald-200' }
    }
  }
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col justify-between text-slate-800">
      <header className="bg-[#0a192f] text-white py-3 px-6 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-sm tracking-wide">GESTÃO DE EQUIPAMENTOS</h1>
            <p className="text-[10px] text-blue-300">MANUTENÇÃO CLÍNICA — FSNH</p>
          </div>
        </div>
        <Link 
          href="/dashboard" 
      ...
      <main className="max-w-6xl w-full mx-auto p-6 flex-1 space-y-6">
        <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          <div>
            <h2 className="text-sm font-bold uppercase text-blue-900">Parque de Equipamentos Hospitalares</h2>
            <p className="text-xs text-slate-500">Controle de patrimônio, número de série, calibrações e localizações.</p>
          </div>
          <button 
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          {loading ? (
            <div className="p-8 text-center text-xs text-slate-500">Carregando equipamentos...</div>
          ) : equipamentos.length === 0 ? (
            <div className="p-8 text-center text-xs text-slate-500">Nenhum equipamento cadastrado até o momento.</div>
          ) : (
                  ...
                  {equipamentos.map((eq) => {
                    ...
                    return (
                      <tr key={eq.id} className="hover:bg-slate-50/80">
                        <td className="p-3 font-mono font-bold text-blue-900">{eq.patrimonio || 'S/N'}</td>
                        <td className="p-3 font-mono text-slate-600">{eq.numeroSerie || eq.serie || 'N/A'}</td>
                        <td className="p-3 font-medium">{eq.tipo || eq.nome || 'Não especificado'}</td>
                        <td className="p-3 text-slate-600">{eq.marca || ''} {eq.modelo || eq.model || ''}</td>
                        <td className="p-3">{eq.setor || 'Geral'}</td>
                        <td className="p-3">
                          {eq.exigeCalibracao ? (
                            statusCalib ? (
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${statusCalib.cor}`}>
                                {statusCalib.texto}
                              </span>
                            ) : (
                            ...
                            <span className="text-[10px] text-slate-400 italic">Não exigida</span>
                          )}
                        </td>
      ...
      {modalAberto && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 my-8">
            <h3 className="text-sm font-bold uppercase text-blue-900 border-b pb-2">Cadastrar Novo Equipamento</h3>
            
            <form onSubmit={handleSalvarNovo} className="space-y-3">
              {/* TIPO DE EQUIPAMENTO COM OPÇÃO DE DIGITAR */}
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Tipo de Equipamento *</label>
                <select 
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Marca / Fabricante</label>
                  <input 
                    type="text"
                    value={novoEq.marca}
                    onChange={e => setNovoEq({...novoEq, marca: e.target.value})}
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Modelo</label>
                  <input 
                    type="text"
                    value={novoEq.modelo}
                    onChange={e => setNovoEq({...novoEq, modelo: e.target.value})}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Número de Série</label>
                  <input 
                    type="text"
                    value={novoEq.numeroSerie}
                    onChange={e => setNovoEq({...novoEq, numeroSerie: e.target.value})}
                    placeholder="Ex: SN-12345"
                    className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2 text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Patrimônio</label>
                  <input 
                    type="text"
                    value={novoEq.patrimonio}
                    onChange={e => setNovoEq({...novoEq, patrimonio: e.target.value})}
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Setor</label>
                <input 
                  type="text"
                  value={novoEq.setor}
                  onChange={e => setNovoEq({...novoEq, setor: e.target.value})}
              ...
              <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    ...
                    onChange={e => setNovoEq({...novoEq, exigeCalibracao: e.target.checked})}
                    className="rounded text-blue-600 focus:ring-blue-500 h-4 w-4"
                  />
                  <span className="text-xs font-bold text-slate-700 uppercase">Este equipamento exige calibração periódica?</span>
                </label>

                {novoEq.exigeCalibracao && (
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200">
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Intervalo (Meses)</label>
                      <input 
                        type="number"
                        min="1"
                        value={novoEq.intervaloCalibracaoMeses}
                        onChange={e => setNovoEq({...novoEq, intervaloCalibracaoMeses: Number(e.target.value)})}
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Data da Última Calibração</label>
                      <input 
                        type="date"
                        value={novoEq.ultimaCalibracao}
                        onChange={e => setNovoEq({...novoEq, ultimaCalibracao: e.target.value})}
                        className="w-full bg-white border border-slate-300 rounded-lg p-1.5 text-xs focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      />
                    </div>
                  </div>
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase text-slate-600 mb-1">Observações, Avarias ou Acessórios Inclusos</label>
                <textarea 
                  rows={2}
                  value={novoEq.observacao}
                  onChange={e => setNovoEq({...novoEq, observacao: e.target.value})}
      ...
      <footer className="text-center py-4 text-[11px] text-slate-500 border-t border-slate-200 bg-white">
